...
function dateDropdown(selector) {
  ...
  $(selector).each((i, el) => {
    ...
    if (selectedDates.days) {
      DPinstance.selectDate([
        dateFromLocaleDateString(selectedDates.from),
        dateFromLocaleDateString(selectedDates.to),
      ]);
    }
